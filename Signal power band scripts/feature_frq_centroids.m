...
% %%Constrict to FEATURE FRQS only
%
% %%createArrayForPCA
%clear all
clc
...
feature_frq = [15,16,19,11,11,9,15,11,11,17];
fwhm = 1;

% Find wavelet span (in time domain)
% NOTE: THIS HAS BEEN MOVED INTO THE FOR LOOPS FOR FEATURE FRQ CALC
...
for i = 1:10
    ...
    % Find wavelet span (in time domain)
    [span] = fwhm2span(feature_frq(i), fwhm); %fwhm2span.m
    [fwhm] = span2fwhm(feature_frq(i), span); %span2fwhm.m
    
    % Gabor wavelet function, ipsi (output is complex signal)
    [gabor_responseFirst_ipsi] = gabor_response_span(dataFirstIpsi, feature_frq(i), span, Fs);
    [gabor_responseLast_ipsi] = gabor_response_span(dataLastIpsi, feature_frq(i), span, Fs);
    %Power envolope, ipsi
...
%     ax.YAxisLocation = 'origin'; %move Y AXIS location to origin
    ...
    title('Avg change in ipsilat Feature Frq position v. ARAT change')
...
for i = 1:10
    ...
    [gabor_responseFirst_contra] = gabor_response_span(dataFirstContra, feature_frq(i), span, Fs);
    [gabor_responseLast_contra] = gabor_response_span(dataLastContra, feature_frq(i), span, Fs);
...
%     ax.YAxisLocation = 'origin'; %move Y AXIS location to origin
    ...
    title('Avg change in contralat Feature Frq position v. ARAT change')
end
hold off 
