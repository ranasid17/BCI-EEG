function [gabor_response] = gpu_gabor_response_span(signal,freqs,spans,fs)
...
%         gpu_conj = gabor;
        gabor_response(f,:,:,:) = gather(convn(signal,gpu_conj,'same'));
        
    else
        gpu_conj = gpuArray(gabor);
%         gpu_conj = gabor;
        gabor_response(f,:,:) = gather(conv2(gpu_conj,1,signal,'same'));

        var_info = whos('signal');
        
        if var_info.bytes > 1e9 
            half_chan_point = round(size(signal,2)/2);
            gabor_response(f,:,1:half_chan_point) = gather(conv2(gpu_conj,1,signal(:,1:half_chan_point),'same'));
            gabor_response(f,:,half_chan_point+1:end) = gather(conv2(gpu_conj,1,signal(:,half_chan_point+1:end),'same'));
